...
export function registerDriveTools(server) {
  ...
  server.tool('drive_search', {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Search query (Drive API format, e.g. "name contains \'report\'" or "mimeType=\'application/vnd.google-apps.spreadsheet\'")' },
        maxResults: { type: 'number', description: 'Max results (default 20)' },
        account: { type: 'string', description: 'Google account email (default: primary)' },
      },
    },
  }, async ({ query, maxResults = 20, account }) => {
  ...
  server.tool('drive_get_file', {
    description: 'Get metadata and content of a Google Drive file. For Docs/Sheets, returns the document content. For other files, returns metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  server.tool('drive_read_sheet', {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        range: { type: 'string', description: 'A1 notation range (e.g. "Sheet1!A1:D10"). Default: first sheet, all data' },
        account: { type: 'string', description: 'Google account email (default: primary)' },
      },
